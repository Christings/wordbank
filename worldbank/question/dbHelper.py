...
class DBHelper():
    ...
    def connectMysql(self):
        conn = pymysql.connect(host=self.host,
                               ...
                               port=self.port,
                               # db=self.db,不指定数据库名
                               charset='utf8')  # 要指定编码，否则中文可能会出现乱码
        return conn
    ...
    # 插入数据
    def insert(self, sql, *params):  # 注意这里params要加*,因为传递过来的是数组，*表示参数个数不定
        conn = self.connectDatabase()
...
class TestDBHelper():
    ...
    # 测试创建数据库(settings配置文件中的MYSQL_DBNAME,直接修改setting配置文件即可)
    def testCreateDatabae(self):
        self.dbHelper.createDatabase()

    # 创建测试表
    def testCreateTable(self):
        sql = "create table worldbank_indicators(id int primary key auto_increment,indi_name_en varchar(255),indi_name_cn varchar(255))"
        self.dbHelper.createTable(sql)

    # 测试插入
    def testInsert(self):
        sql = "insert into worldbank_indicators(indi_name_en,indi_name_en) values(%s,%s)"
        params = ("test", "test")
        self.dbHelper.insert(sql, *params)  # *表示拆分元组，调用insert（*params）会重组成元组

    def testUpdate(self):
        sql = "update worldbank_indicators set indi_name_en=%s,indi_name_en=%s where id=%s"
        params = ("update", "update", "1")
        self.dbHelper.update(sql, *params)

    def testDelete(self):
        sql = "delete from worldbank_indicators where id=%s"
        params = ("1")
